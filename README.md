...
2. For now Gou doesn't consider synchronism(mutex etc). Caches in disk may be broken. This problem would be fixed if everyone thinks Gou is useful :) .
3. Gou can try to open port by uPnP and NAT-PMP. You can disable this function by setting enable_nat:false in [Gateway]  in saku.ini, which is true by default.
